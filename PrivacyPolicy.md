...
- **`activeTab`**:
  - **Why we need it**: To allow the "Spotlight" feature (opened via `Alt+P` or clicking the icon) to appear over your current webpage, and to fetch page details (like title or URL) if you use "Page Element" variables.
  - **How it works**: This permission grants the extension temporary access to the currently active tab _only when you explicitly invoke the extension_. It does not allow us to monitor your browsing history in the background.
- **`scripting`**:
  - **Why we need it**: To verify if the text insertion logic (`content.js`) is loaded, to execute it when necessary, and to fetch data for "Page Element" variables.
  - **How it works**: When you use the "Insert" function, the extension programmatically inserts your text into the active input field. If you use "Advanced Variables", it may also execute a script to retrieve the text of a specific element (like `h1`) based on your configuration. All user-defined scripts run in a restricted **Sandbox** environment.
- **`sidePanel`**:
