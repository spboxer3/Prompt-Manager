...
- **智慧變數系統**：
  ...
  - **進階變數 (Advanced v1.2.0)**：
    - **頁面元素 (Page Elements)**：使用 CSS 選擇器自動抓取網頁資料 (例如 `{{pageTitle}}` -> `document.title`)。
    - **腳本 (Script)**：在安全的沙箱環境執行 JavaScript 進行計算 (例如日期, 文字處理)，並支援以唯讀方式存取網頁資料 (`document` 物件)。
...
- **進階變數 (v1.2 新功能)**：
  - **頁面元素 (Selector)**：定義變數如 `page_h1` = `h1` (Selector)。它會自動抓取當前頁面的 H1 標題。
  - **腳本 (Script)**：定義 `today` = `return new Date().toLocaleDateString();`。這會執行 JS 並填入結果。您甚至可以使用 `document` (唯讀) 來存取網頁內容！
